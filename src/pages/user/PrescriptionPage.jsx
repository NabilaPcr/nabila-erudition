...
const demoHistory = [
  {
    ...
    status: 'selesai',
    notes: 'Resep telah diproses dan obat siap diambil.'
  },
  {
    id: 'RES-002',
    date: '28 Jun 2026',
    doctor: 'dr. Sari Wulandari',
    items: 'Vitamin D3, Zinc Tablet',
    status: 'diproses',
    notes: 'Sedang diverifikasi oleh apoteker.'
  },
];

const statusCfg = {
  selesai:  { label: 'Selesai',  bg: '#f0fdf4', text: '#16a34a', icon: <CheckIcon /> },
  diproses: { label: 'Diproses', bg: '#fffbeb', text: '#92400e', icon: null },
  ditolak:  { label: 'Ditolak', bg: '#fef2f2', text: '#991b1b', icon: <XIcon /> },
};
...
export default function PrescriptionPage() {
  ...
  return (
    <div className="animate-fade-in max-w-3xl">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-gray-900 m-0">Resep Saya</h1>
        <p className="text-sm text-gray-400 m-0 mt-1">Upload foto resep dokter untuk mendapatkan obat resep</p>
      </div>

      {/* Upload Form */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
        <h2 className="text-base font-bold text-gray-900 mb-5">Upload Resep Baru</h2>
        ...
        {submitted ? (
          <div className="text-center py-10">
            ...
            <h3 className="font-bold text-gray-900 mb-1">Resep Berhasil Dikirim!</h3>
            <p className="text-sm text-gray-400">Apoteker akan memverifikasi resep Anda dalam 1x24 jam.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div
              ...
              style={{ borderColor: dragging ? '#22c55e' : '#e5e7eb', background: dragging ? '#f0fdf4' : '#f9fafb' }}
              onClick={() => document.getElementById('resep-file').click()}
            >
              {preview ? (
                <div className="relative">
                  <img src={preview} alt="Preview resep" className="max-h-48 max-w-full mx-auto rounded-xl object-cover mb-3" />
                ...
                <>
                  <div className="flex justify-center text-gray-300 mb-3"><UploadIcon /></div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">Seret foto resep ke sini</p>
                  <p className="text-xs text-gray-400 mb-3">atau klik untuk memilih file (JPG, PNG, PDF)</p>
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
              <p className="text-xs font-bold text-blue-700 mb-1">Informasi Penting</p>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-bold text-gray-900 m-0">Riwayat Resep</h2>
        </div>
              ...
              return (
                <div key={r.id} className="px-6 py-5">
                  <div className="flex items-start justify-between gap-4 mb-2">
                    <div>
                      <p className="text-xs text-gray-400 font-medium m-0">{r.id} · {r.date}</p>
                      <p className="text-sm font-bold text-gray-900 m-0 mt-0.5">Resep dari {r.doctor}</p>
                      <p className="text-xs text-gray-500 m-0 mt-1">{r.items}</p>
                    </div>
                  ...
                  {r.notes && (
                    <p className="text-xs text-gray-400 mt-2 bg-gray-50 rounded-lg px-3 py-2">{r.notes}</p>
                  )}
                </div>
              );
            })}
